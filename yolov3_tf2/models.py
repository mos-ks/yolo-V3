...
flags.DEFINE_float("yolo_iou_threshold", 0.6, "iou threshold")
...
def yolo_nms(outputs, anchors, masks, classes):
    ...
    selected_indices = tf.concat(
        [
            selected_indices,
            tf.zeros(FLAGS.yolo_max_boxes - num_valid_nms_boxes, tf.int32),
        ],
        0,
    )
    selected_scores = tf.concat(
        [
            selected_scores,
            tf.zeros(FLAGS.yolo_max_boxes - num_valid_nms_boxes, tf.float32),
        ],
        -1,
    )
